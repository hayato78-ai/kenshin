...
function onOpen() {
  ...
  ui.createMenu('健診システム')
    ...
    .addSubMenu(ui.createMenu('👤 受診者管理')
      ...
      .addSeparator()
      .addItem('検索テスト', 'testShowPatientSearchDialog'))
    .addSeparator()
    // CSVインポート（AIマッピング対応）メニュー（Phase 3追加）
    .addSubMenu(ui.createMenu('📥 CSVインポート')
      .addItem('CSVインポート（AIマッピング）', 'showCsvImportDialog')
      .addSeparator()
      .addItem('AIマッピングテスト', 'testShowAiMappingDialog'))
    .addSeparator()
    // Python連携Excel出力メニュー
    ...
    .addSeparator()
    // 組織マスタ管理メニュー（Phase 3追加）
    .addSubMenu(ui.createMenu('🏢 組織マスタ')
      .addItem('健康保険組合一覧', 'showHealthInsuranceList')
      .addItem('事業所一覧', 'showBusinessOfficeList')
      .addItem('企業一覧', 'showCompanyList')
      .addSeparator()
      .addItem('検診種別一覧', 'showExamTypeList')
      .addItem('コース一覧', 'showCourseList')
      .addSeparator()
      .addItem('組織マスタシート作成', 'createOrganizationMasterSheets'))
    .addSeparator()
    // 帳票・レポート出力メニュー（Phase 4追加）
    .addSubMenu(ui.createMenu('📊 帳票出力')
      .addItem('企業別健診結果一覧', 'showCompanyReportDialog')
      .addSeparator()
